...
########################################################################################################
hist( MyNewData2 $DEVELOPED )


summary( lm(NETMIG~PTotAge65+ PUnempolyed + PBelPoverty+ PMinority + policyCost + 
	policyCount + LogNPMDamount  + LogSBALoan + LogPropertyDmg + Fatality + 
	CropDmglog + LogHAOAproved, data =MyNewData2) )
#The ols_vif_tol function from the olsrr package in R is used to detect multicollinearity in a 
#regression model. Multicollinearity occurs when two or more predictors in the model are highly 
#correlated, making it difficult to determine the individual effect of each predictor on the 
#dependent variable.

#Here is what the function provides:

#Variance Inflation Factor (VIF): VIF measures how much the variance of an estimated 
#regression coefficient increases if your predictors are correlated. A VIF value 
#greater than 10 (or sometimes 5) is often considered an indication of high multicollinearity.

#Tolerance (TOL): Tolerance is the reciprocal of VIF (1/VIF). It indicates the percentage of 
#the variance in a predictor that cannot be explained by other predictors. A tolerance value 
#less than 0.1 is often taken as an indicator of serious multicollinearity.

#The ols_vif_tol function will return a dataframe with the following columns 
#for each predictor in your model:

#Variables: Names of the predictors.
#Tolerance: The tolerance value for each predictor.
#VIF: The variance inflation factor for each predictor.
########################################################################################################
